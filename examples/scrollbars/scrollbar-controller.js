 ...
 * 
 * Contents of this file should be attached to AxHooxPrepareMasterContext event
 * of axhoox-status or axhoox starter masters
 * 
 * Controller instance when placed somewhere inside the controled scrollbar
 * master examines if it's horizontal or vertical scrollbar
 * 
 */
...
var OWNER_API_NAMES =
        [ 'activate', 'enable', 'disable', 'attachContent', 'getScroll',
                'setScroll', 'onHandleHover', 'onHandleOut',
                'onHandleDragStart', 'onHandleDrag', 'onHandleDragDrop',
                'maxScroll', 'barMaxScroll' ];
...
masterContext && $.extend(masterContext, API) ||
...
function activate() {
    ...
    if (!this.data.active) {
        ...
        this.data.bar.get('dp-main').setState('active', 'fade', '', 300,
                'fade', '', 300);
    }
    return this;
}
...
function deactivate(force) {
    ...
    if (this.data.active || force) {
        ...
        this.data.bar.get('dp-main').setState('idle', 'fade', '', 300, 'fade',
                '', 300);
    }
    return this;
}
...
function onHandleOut() {
    ...
    if (this.data.handleDrag) {
        ...
        this.cancelDelayedDeactivation();
        this.data.ddId =
                setTimeout(delayedDeactivation.bind(this), deactivationDelay);
    }
    return this;
}
...
function _updateContentPosition() {
    ...
    moveByParams[this.data.barType] =
            -this.data.scroll - this.data.content.getRect()[coord];
...
function _updateHandlePosition() {
    ...
    var newHandlePos =
            this.data.scroll * this.data.barMaxScroll / this.data.maxScroll;
    var moveByParams = [ 0, 0 ];
...
function onHandleDragDrop() {
    ...
    var hl = h.getRect()[_dimmensions[this.data.barType + 2]]; // handle
    // location
    var d =
            hl < 0 ? -hl : hl > this.data.barMaxScroll ?
                    this.data.barMaxScroll - hl : 0;
    ...
    if (this.data.shouldDeactivateOnDrop) {
        // run condition is happening sometimes and that's why we have forced
        // deactivation
